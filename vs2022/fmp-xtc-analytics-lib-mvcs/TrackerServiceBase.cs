...
namespace XTC.FMP.MOD.Analytics.LIB.MVCS
{
    ...
    /// Tracker服务层基类
    /// </summary>
    public class TrackerServiceBase : Service
    {
        public TrackerServiceMock mock { get; set; } = new TrackerServiceMock();
        ...
        public TrackerServiceBase(string _uid, string _gid) : base(_uid)
        {
            gid_ = _gid;
        }
        ...
        /// <returns>错误</returns>
        public virtual async Task<Error> CallRecord(TrackerRecordRequest? _request, object? _context)
        {
        ...
        protected Tracker.TrackerClient? getGrpcClient()
        {
            if (null == grpcChannel_)
                return null;

            if(null == clientTracker_)
            {
                clientTracker_ = new Tracker.TrackerClient(grpcChannel_);
            }
            return clientTracker_;
        }
        ...
        /// GRPC客户端
        /// </summary>
        protected Tracker.TrackerClient? clientTracker_;
