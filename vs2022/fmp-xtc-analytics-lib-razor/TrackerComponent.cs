...
namespace XTC.FMP.MOD.Analytics.LIB.Razor
{
    ...
    {
        private TrackerFacade? getFacade()
        {
            if (null == facade_)
            {
                if (null == framework_)
                ...
                var entry = framework_.getUserData("XTC.FMP.MOD.Analytics.LIB.MVCS.Entry") as Entry;
                if (null == entry)
                {
                ...
                facade_ = entry?.getDynamicTrackerFacade("default");
                if (null == facade_)
                {
                    logger_?.Error("facade_ is null");
                    return null;
                }
                facade_?.setUiBridge(new TrackerUiBridge(this));
            }
            return facade_;
        }
