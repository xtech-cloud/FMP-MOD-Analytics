...
namespace XTC.FMP.MOD.Analytics.LIB.Unity
{
    public class MyInstanceBase
    {
        ...
        public IGeneratorViewBridge viewBridgeGenerator { get; set; }

        public IHealthyViewBridge viewBridgeHealthy { get; set; }

        public ITrackerViewBridge viewBridgeTracker { get; set; }
        ...
        public void SetupBridges()
        {
            ...
            var facadeGenerator = entry_.getDynamicGeneratorFacade(uid);
            var bridgeGenerator = new GeneratorUiBridge();
            bridgeGenerator.logger = logger_;
            facadeGenerator.setUiBridge(bridgeGenerator);
            viewBridgeGenerator = facadeGenerator.getViewBridge() as IGeneratorViewBridge;
            ...
            var facadeTracker = entry_.getDynamicTrackerFacade(uid);
            var bridgeTracker = new TrackerUiBridge();
            bridgeTracker.logger = logger_;
            facadeTracker.setUiBridge(bridgeTracker);
            viewBridgeTracker = facadeTracker.getViewBridge() as ITrackerViewBridge;
        ...
        protected virtual void submitGeneratorRecord(GeneratorRecordRequest _request)
        {
            var dto = new GeneratorRecordRequestDTO(_request);
            SynchronizationContext context = SynchronizationContext.Current;
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgeGenerator.OnRecordSubmit(dto, context);
        ...
        protected virtual void submitTrackerRecord(TrackerRecordRequest _request)
        {
            var dto = new TrackerRecordRequestDTO(_request);
            SynchronizationContext context = SynchronizationContext.Current;
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgeTracker.OnRecordSubmit(dto, context);
                    if (!LibMVCS.Error.IsOK(reslut))
                    {
                        logger_.Error(reslut.getMessage());
                    }
